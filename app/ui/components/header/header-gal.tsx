...
// "use client" nécessaire : on a maintenant un état (menu ouvert/fermé)
// et des listeners (clic extérieur, touche Échap).
export default function HeaderGal({ collections, currentSlug }: HeaderGalProps) {
  ...
  return (
    <nav className="flex flex-row justify-around items-center bg-neutral-400 items-center gap-3 px-6 py-8 text-center sm:px-10">
      {/* Élément 1 : titre de la collection en cours */}
      <h1
        className={`${cinzel.className} text-2xl tracking-wide text-gray-900 sm:text-3xl`}
      >
        {current?.title ?? "Collection"}
      </h1>

      {/* Élément 2 : menu déroulant pour changer de collection */}
